...
const int MAX_RESULT_DOCUMENT_COUNT = 6;
// Lalalalalalal
//lalallalala
////lalalalal
...
struct Document {
    ...
    DocumentStatus status;
     ...
     tuple<DocumentStatus, int, double> MakeKey() const {
...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query,
        ...
        //Проверка на совпадения статуса
        erase_if(matched_documents,
            [&status](const Document& doc) {return doc.status != status; });
    ...
    tuple<vector<string>, DocumentStatus> MatchDocument(const string& raw_query, int document_id) const {
        ...
        for (const auto& plus : query.plus_words) {
            if (word_to_document_freqs_.count(plus) != 0 && 
                word_to_document_freqs_.at(plus).count(document_id)!= 0){
        ...
        return { result, document_status_.at(document_id) };
    }
